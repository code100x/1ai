...
import { useChatContextHook } from "@/contexts/chat-context";
...
const UIInput = ({
  ...
  const wrapperRef = useRef<HTMLDivElement>(null); // Added for focusability

  // gettin from the context
  const { textRef } = useChatContextHook();

  return (
    <div
      tabIndex={-1}
      ref={wrapperRef}
      className="flex w-full max-w-screen flex-col items-center justify-center gap-4"
    >
