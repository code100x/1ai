...
import { YoutubeSummarizer } from "./youtube-summarizer/youtube-summarizer";
...
router.get(
  "/article-summarizer/:executionId",
  authMiddleware,
  async (req, res) => {
...
router.post("/article-summarizer", authMiddleware, (req, res) => {
  ...
  articleSummarizer
    .runStreamable(result.data as any, (chunk: string) => {
...
router.get(
  "/youtube-summarizer/:executionId",
  authMiddleware,
  async (req, res) => {
    const execution = await prismaClient.execution.findFirst({
      where: {
    ...
    res.json({
      ...
      videoId: youtubeSummarizer.videoId,
